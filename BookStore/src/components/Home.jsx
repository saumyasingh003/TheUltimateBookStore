...
function Banner() {
  ...
  const [statusMessage, setStatusMessage] = useState(""); // State for showing status

  // Function to handle form submission
  const handleGetStarted = async () => {
    try {
      const response = await axios.post(
        "https://book-store-backend-ul1r.onrender.com/mail/send-email",
        {
          email,
        }
      );
    ...
    <>
      <div className="max-w-screen-2xl container mx-auto lg:px-40 md:px-20 px-10 md:mt-20 flex flex-col md:flex-row">
        <div className="w-full order-2 md:order-1 md:w-1/2 mt-8 md:mt-24">
          ...
          <button className="btn mt-4 btn-success" onClick={handleGetStarted}>
            Get Started
          </button>
          {statusMessage && (
            <p className="mt-2 text-green-600">{statusMessage}</p>
          )}
        </div>
